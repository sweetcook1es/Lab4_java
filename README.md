...
+ put(item) — проверяем, пуста ли коробка, если нет — бросаем исключение
...
### Алгоритм:
+ Создаем интерфейс Comparable<T> с методом compare(T other)
+ Реализуем класс ComparableInteger для сравнения чисел
+ В методе compare используем стандартное сравнение Integer.compareTo()
