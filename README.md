...
### Алгоритм:
+ Создаем класс Box<T> с полем content
+ put(item) — проверка, пуста ли коробка, если нет — исключение
+ get() — возвращаем содержимое и обнуляем поле
+ isEmpty() — проверяем, равно ли поле null
...
### Алгоритм:
+ интерфейс Comparable<T> с методом compare(T other)
+ класс ComparableInteger для сравнения чисел
+ В методе compare стандартное сравнение Integer.compareTo()
...
### Алгоритм:
+ Метод принимает список Box<? extends Number>
+ Для каждой коробки извлекаем значение через get()
+ Преобразуем в double через doubleValue()
...
### Алгоритм:
+ Интерфейс ApplyFunction<T, R> с методом apply(T t)
+ Создаем новый список для результатов
+ Для каждого элемента исходного списка вызываем function.apply(item)
+ Результат добавляем в новый список
...
### Алгоритм:
+ Интерфейс TestFunction<T> с методом test(T t)
+ Создаем новый список
+ Для каждого элемента проверяем predicate.test(item)
+ Если условие истинно — добавляем в результат
...
### Алгоритм:
+ Интерфейс ReduceFunction<T> с методом apply(accumulator, current)
+ Инициализируем accumulator начальным значением
+ Для каждого элемента списка: accumulator = reducer.apply(accumulator, item)
...
### Алгоритм:
+ Два интерфейса: Supplier<P> (создает контейнер) и CollectorFunction<T, P> (добавляет элемент)
+ Создаем контейнер: container = supplier.get()
+ Для каждого элемента: collector.apply(container, item)
